...
class ComputoMetricoDialog(QWidget):
    def __init__(self, iface, plugin_dir: str, parent=None):
        ...
        self.data_dir = os.path.join(
            QgsApplication.qgisSettingsDirPath(), "computo_metrico_gis"
        )
    ...
    def _build_ui(self):
        ...
        title = QLabel(
            ...
            "<div style='color:#9eb6d5'>Computo metrico estimativo da QGIS "
            "con prezziari e report.</div>"
        )
    ...
    def _build_info_tab(self):
        ...
        for button in (
            self.btn_init_db,
            self.btn_create_template,
            self.btn_refresh_context,
        ):
    ...
    def _build_price_tab(self):
        ...
        self._configure_table(
            self.region_catalog_table, stretch_column=0, min_height=250
        )
        ...
        official_note = QLabel(
            "Il plugin prova a risolvere il prezziario ufficiale della "
            "regione selezionata, "
            "lo salva in una cartella locale del plugin e importa subito i "
            "formati leggibili."
        )
        ...
        self.btn_open_bundled_source = QPushButton(
            "Apri sorgente aggiornamento"
        )
        ...
        self.download_result_browser.setHtml(
            ...
            "<p>Qui compariranno il percorso locale, il link al file e il "
            "link alla cartella.</p>"
        )
        ...
        self.download_links_table.setHorizontalHeaderLabels(
            ["Etichetta", "Regione/ente", "URL"]
        )
        self._configure_table(
            self.download_links_table, stretch_column=2, min_height=240
        )
        ...
        self._configure_table(
            self.price_items_table, stretch_column=1, min_height=320
        )
        ...
        self.btn_catalog_create_template.clicked.connect(
        ...
        self.btn_download_official.clicked.connect(
        ...
        self.bundled_dataset_combo.currentIndexChanged.connect(
            self.refresh_bundled_dataset_info
        )
        self.region_catalog_table.itemSelectionChanged.connect(
            self.refresh_catalog_details
        )
        self.price_list_combo.currentIndexChanged.connect(
    ...
    def _build_measurement_tab(self):
        ...
        self.measurement_help_browser.setHtml(
            ...
            "Gli altri campi sono facoltativi ma utili per controllare il "
            "computo.</p>"
            "<ul>"
            "<li><b>mq / ha</b>: usa l'area della geometria.</li>"
            "<li><b>mq da linea</b>: se imposti <code>larghezza_m</code>, usa "
            "lunghezza x larghezza.</li>"
            "<li><b>ml / km</b>: usa la lunghezza, o il perimetro se il layer "
            "&egrave; poligonale.</li>"
            "<li><b>cad</b>: conta una feature = una quantità.</li>"
            "<li><b>mc</b>: usa area x spessore oppure lunghezza x larghezza "
            "x altezza/spessore.</li>"
            "</ul>"
        )
    ...
    def _build_report_tab(self):
        ...
        self.report_include_map_checkbox = QCheckBox(
            "Includi la mappa corrente nel PDF"
        )
        ...
        self.report_footer_edit.setPlaceholderText(
            "Piè di pagina, note finali, riferimenti normativi sintetici."
        )
        ...
        self.report_refs_table.setHorizontalHeaderLabels(
            ["Etichetta", "Valore"]
        )
        self._configure_table(
            self.report_refs_table, stretch_column=1, min_height=180
        )
        ...
        self._configure_action_button(
            self.btn_save_report_profile, min_width=220
        )
        ...
        self._configure_table(
            self.summary_table, stretch_column=1, min_height=280
        )
        ...
        self.detail_table.setHorizontalHeaderLabels(
            [
                ...
                "P. Unit (€)",
                "Totale (€)",
                "Regola e Note calcolo",
            ]
        )
        self._configure_table(
            self.detail_table, stretch_column=9, min_height=300
        )
    ...
    def _build_accounting_tab(self):
        ...
        sal_form.addRow(
            "Già pagato / certificato", self.sal_previous_paid_edit
        )
        ...
        self._configure_browser(
            self.accounting_certificate_browser, min_height=180
        )
        ...
        self._configure_table(
            self.accounting_libretto_table, stretch_column=8, min_height=300
        )
        ...
        self._configure_table(
            self.accounting_registro_table, stretch_column=1, min_height=300
        )
        ...
        self.accounting_sommario_table.setHorizontalHeaderLabels(
            ["Categoria", "Importo"]
        )
        self._configure_table(
            self.accounting_sommario_table, stretch_column=0, min_height=260
        )
        ...
        self._configure_table(
            self.accounting_sal_table, stretch_column=2, min_height=260
        )
        ...
        self.journal_date_edit = QLineEdit(
            QDateTime.currentDateTime().toString("yyyy-MM-dd")
        )
        ...
        self._configure_table(
            self.accounting_journal_table, stretch_column=4, min_height=260
        )
        ...
        self.btn_refresh_accounting.clicked.connect(
            self.refresh_accounting_tab
        )
        self.accounting_run_combo.currentIndexChanged.connect(
            self.refresh_accounting_tab
        )
        ...
        self.btn_export_accounting_pdf.clicked.connect(
            self.export_accounting_pdf
        )
        self.btn_export_accounting_xlsx.clicked.connect(
            self.export_accounting_workbook
        )
        self.btn_add_journal_entry.clicked.connect(self.add_journal_entry)
        self.btn_delete_journal_entry.clicked.connect(
            self.delete_journal_entry
        )
        self.accounting_sal_table.itemSelectionChanged.connect(
            self.load_selected_sal_into_form
        )
        return self._wrap_scroll_tab(tab, min_width=1320)
    ...
    def _configure_action_button(
        self, button: QPushButton, min_width: int = 180
    ):
    ...
    def _configure_table(
        ...
        table.setProperty(
            "stretch_column", -1 if stretch_column is None else stretch_column
        )
        if stretch_column is not None:
            table.horizontalHeader().setSectionResizeMode(
                stretch_column, header_stretch_mode()
            )
    ...
    def initialize_database(self):
        ...
        QMessageBox.information(
            self, PLUGIN_NAME, "Database SQLite inizializzato correttamente."
        )
    ...
    def refresh_info_tab(self):
        ...
        info_html = info_html.replace(
            "__PRICE_LISTS__", str(price_lists_count)
        )
    ...
    def refresh_price_lists(self, selected_id: int | None = None):
        ...
        active_id = selected_id or self._active_setting_int(
        ...
        if current_index >= 0:
            ...
            self.db.set_setting(
                "active_price_list_id", self.price_list_combo.currentData()
            )
    ...
    def refresh_regional_catalog(self):
        ...
        self.region_catalog_table.setRowCount(
            len(self.regional_catalog_entries)
        )
        ...
        for row_index, entry in enumerate(self.regional_catalog_entries):
            ...
            for column_index, value in enumerate(values):
                ...
                self.region_catalog_table.setItem(
                    row_index, column_index, item
                )

        self.region_catalog_table.resizeColumnsToContents()
        self.region_catalog_table.horizontalHeader().setSectionResizeMode(
            0, header_stretch_mode()
        )
    ...
    def refresh_catalog_details(self):
        ...
        if not entry:
            self.region_catalog_browser.setHtml(
                "<p>Nessuna regione selezionata.</p>"
            )
            return
        ...
        if entry.local_dataset:
            ...
            local_dataset_line = (
                f'<p><b>Dataset locale pronto:</b> <a '
                f'href="{dataset_url}">{entry.local_dataset}</a></p>')

        local_download_line = ""
        if entry.local_download:
            local_download_url = QUrl.fromLocalFile(
                entry.local_download
            ).toString()
            local_download_line = (
                f'<p><b>Ultimo file scaricato:</b> <a '
                f'href="{local_download_url}">{entry.local_download}</a></p>')

        manual_dir = str(Path(self.data_dir) / "templates_regionali")
        manual_dir_url = QUrl.fromLocalFile(manual_dir).toString()
        status_line = (
            f'<p><b>Stato:</b> <span style="color:{entry.status_color}">'
            f'{entry.status_label}</span></p>'
        )
        source_page = entry.source_page or entry.homepage
        source_page_line = (
            f'<p><b>Pagina ufficiale:</b> <a href="{source_page}">'
            f'{source_page}</a></p>'
        )
        update_line = (
            f'<a href="{entry.update_url}">{entry.update_url}</a>'
            if entry.update_url
            else "non disponibile"
        )
        update_line = f"<p><b>Link aggiornamento:</b> {update_line}</p>"
        manual_dir_line = (
            f'<p><b>Cartella template manuali:</b> '
            f'<a href="{manual_dir_url}">{manual_dir}</a></p>'
        )
        notes = entry.notes or (
            "Portale regionale censito. Se il dataset non è pronto, usa "
            "il download ufficiale o il template manuale."
        )
        notes_line = f"<p><b>Note:</b> {notes}</p>"
        self.region_catalog_browser.setHtml(f"""
    ...
    def refresh_bundled_dataset_info(self):
        ...
        local_file_line = (
            f'<p><b>File locale incluso:</b> <a href="{file_url}">'
            f'{local_path}</a></p>'
        )
        source_page_line = (
            f'<p><b>Pagina ufficiale:</b> <a href="{dataset.source_page}">'
            f'{dataset.source_page}</a></p>'
        )
        update_line = (
            f'<p><b>Link aggiornamento:</b> '
            f'<a href="{dataset.update_url}">{dataset.update_url}</a></p>'
        )
        self.bundled_info_browser.setHtml(f"""
    ...
    def refresh_download_links(self):
        ...
        for row_index, row in enumerate(rows):
            for column_index, key in enumerate(("label", "region", "url")):
                ...
                self.download_links_table.setItem(
                    row_index, column_index, item
                )
        self.download_links_table.resizeColumnsToContents()
        self.download_links_table.horizontalHeader().setSectionResizeMode(
            2, header_stretch_mode()
        )
    ...
    def refresh_layers(self):
        ...
        for layer in vector_layers:
            self.layer_combo.addItem(
                f"{layer.name()} [{layer.crs().authid()}]", layer.id()
            )
        if current_id:
    ...
    def auto_detect_fields(self):
        ...
        mapping = {
            ...
            self.description_field_combo: guess_field_name(
                layer, "description"
            ),
            self.category_field_combo: guess_field_name(layer, "category"),
            self.quantity_field_combo: guess_field_name(layer, "quantity"),
            self.coefficient_field_combo: guess_field_name(
                layer, "coefficient"
            ),
            self.unit_override_field_combo: guess_field_name(
                layer, "unit_override"
            ),
    ...
    def import_xlsx(self):
        path, _ = QFileDialog.getOpenFileName(
            ...
            str(Path.home()),
            "Prezziari (*.xlsx *.xlsm *.dcf);;Excel (*.xlsx *.xlsm);;DCF "
            "(*.dcf)",
        )
        if path:
            self.import_price_list_from_path(path)

    def import_price_list_from_path(
        self, path: str, source_url: str = "", page_url: str = ""
    ):
        try:
            ...
            QMessageBox.critical(
                self, PLUGIN_NAME, f"Import non riuscito:\n{exc}"
            )
        ...
        QMessageBox.information(
            self,
            PLUGIN_NAME,
            f"Prezziario '{price_list.name}' importato con "
            f"{len(price_list.items)} voci.",
        )
        self.log(
            f"Prezziario importato: {price_list.name} "
            f"({len(price_list.items)} voci)"
        )
    ...
    def import_bundled_dataset(self):
        ...
        if not dataset:
            QMessageBox.information(
                self, PLUGIN_NAME, "Nessun dataset incluso disponibile."
            )
            return
        local_path = str(Path(self.plugin_dir) / dataset.relative_path)
    ...
    def import_catalog_dataset(self):
        ...
        if not entry:
            QMessageBox.information(
                self, PLUGIN_NAME, "Seleziona prima una regione dal catalogo."
            )
            return
        if entry.local_dataset:
            self.import_price_list_from_path(
            ...
            return
        if entry.local_download and Path(
            entry.local_download
        ).suffix.lower() in {".csv", ".xlsx", ".xlsm", ".zip", ".dcf"}:
            self.import_price_list_from_path(
                entry.local_download,
                source_url=entry.update_url,
                page_url=entry.source_page,
        ...
        QMessageBox.information(
            self,
            PLUGIN_NAME,
            "Per questa regione non è ancora presente un dataset locale "
            "importabile.\n\n"
    ...
    def download_catalog_entry(self):
        ...
        if entry.update_url:
            try:
                self._start_download_feedback(
                    f"Download del file ufficiale per {entry.name}..."
                )
                ...
                self.log(
                    f"Scaricato file catalogato per {entry.name}: "
                    f"{downloaded_path}"
                )
                if Path(downloaded_path).suffix.lower() in {
                    ".csv",
                    ".xlsx",
                    ".xlsm",
                    ".zip",
                    ...
                    QMessageBox.information(
                        self,
                        PLUGIN_NAME,
                        "File ufficiale scaricato correttamente, ma non "
                        "importabile in automatico.\n\n"
                        "Puoi aprirlo dal pannello catalogo oppure compilare "
                        "il template manuale.",
                    )
                return
            except Exception as exc:
    ...
    def open_catalog_file(self):
        ...
        if not path:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Nessun file locale disponibile per questa regione. Puoi "
                "scaricarlo oppure creare il template manuale.",
            )
            return
    ...
    def add_download_link(self):
        label, ok = QInputDialog.getText(
            self, "Nuovo link", "Etichetta del prezziario"
        )
        if not ok or not label.strip():
            return
        url, ok = QInputDialog.getText(
            self, "Nuovo link", "URL diretto di download"
        )
        if not ok or not url.strip():
            return
        region, ok = QInputDialog.getText(
            self, "Nuovo link", "Regione o ente (opzionale)"
        )
        if not ok:
            region = ""
    ...
    def remove_download_link(self):
        ...
        if not link_id:
            QMessageBox.information(
                self, PLUGIN_NAME, "Seleziona prima un link dalla tabella."
            )
            return
    ...
    def download_selected_link(self):
        ...
        try:
            ...
            downloaded_path = self._download_to_temp(
                url, output_dir=self.download_dir, source_label="manuale"
            )
            ...
            QMessageBox.warning(
                self,
                "Formato non valido",
                f"{exc}\n\n{exc.details}\n\nIl file scaricato resta comunque "
                f"disponibile nel pannello 'Stato download'.",
            )
            self.log(f"Download non importabile: {exc}")
        except Exception as exc:
            QMessageBox.critical(
                self, PLUGIN_NAME, f"Download non riuscito:\n{exc}"
            )
    ...
    def download_official_pricelist(self):
        ...
        if not region_key:
            QMessageBox.information(
                self, PLUGIN_NAME, "Seleziona una regione."
            )
            return
        try:
            self._start_download_feedback(
                "Ricerca del prezziario ufficiale..."
            )
            ...
            self._finish_download_feedback(
                "Download non riuscito.", failed=True
            )
            QMessageBox.warning(
                self,
                "Download ufficiale non risolto",
                f"{exc}\n\nPuoi comunque usare il caricamento manuale o "
                f"registrare un link diretto.",
            )
        ...
        self.log(
            f"Scaricato prezziario ufficiale {result['source_name']}: "
            f"{result['file_path']}"
        )
        self._set_download_result(
            ...
            title=(
                f"Download ufficiale completato per "
        ...
        self._finish_download_feedback(
            "File scaricato ma non importabile automaticamente."
        )
        self.refresh_regional_catalog()
        QMessageBox.information(
            self,
            ...
            "Il portale ufficiale ha pubblicato un formato non ancora "
            "importabile in automatico "
            "(tipicamente PDF). Il file resta disponibile per consultazione o "
            "conversione manuale.",
        )
    ...
    def _download_to_temp(
        self, url: str, output_dir: str = "", source_label: str = ""
    ) -> str:
        ...
        with urlopen(
            ...
            if "text/html" in content_type and suffix not in (
                ".csv",
                ".xlsx",
                ".zip",
                ".pdf",
                ...
                raise RuntimeError(
                    "L'URL non sembra puntare a un download diretto."
                )
            if not suffix:
            ...
            if output_dir:
                ...
                file_name = (
                    ...
                    or "prezzario_scaricato"
                )
                file_name = self._sanitize_download_name(
                    file_name, suffix, source_label
                )
                path = str(Path(output_dir) / file_name)
                temp_mode = False
            else:
                fd, path = tempfile.mkstemp(
                    prefix="computo_prezziario_", suffix=suffix
                )
                os.close(fd)
                temp_mode = True
            ...
            with open(path, "wb") as output_handle:
                while True:
                    ...
                    self._update_download_progress(
                        downloaded,
                        total,
                        f"Scaricati {downloaded} byte"
                        + (f" di {total}" if total else ""),
                    )
    ...
    def run_computation(self):
        ...
        if not layer:
            QMessageBox.warning(
                self, PLUGIN_NAME, "Seleziona un layer vettoriale."
            )
            return
        if not price_list_id:
            QMessageBox.warning(
                self, PLUGIN_NAME, "Importa o seleziona un prezziario attivo."
            )
            self.tabs.setCurrentWidget(self.price_tab)
            return
        ...
        mapping = MappingConfig(
            ...
            unit_override_field=self._combo_value(
                self.unit_override_field_combo
            ),
        ...
        if layer.crs().isGeographic():
            warnings.insert(
                0,
                "Il layer usa un CRS geografico. Per computi affidabili è "
                "preferibile un CRS metrico proiettato.",
            )

        if not items:
            QMessageBox.warning(
                self, PLUGIN_NAME, "Nessuna feature valida da computare."
            )
            self.measurement_feedback.setHtml(
                "<p><b>Nessun risultato.</b> Controlla il mapping e i codici "
                "voce.</p>"
            )
            return

        notes = (
            f"Mapping codice={mapping.code_field}; "
            f"descrizione={mapping.description_field}; "
            f"quantita_man={mapping.quantity_field}; "
            f"coefficiente={mapping.coefficient_field}"
        )
        ...
        rows_html = "".join(
            ...
            f"<td><b>{it.quantity} {it.unit}</b></td><td>€ "
            f"{it.unit_price:.2f}</td>"
            f"<td><b>€ {it.total_price:.2f}</b></td><td "
            f"style='font-size:10px;color:#334155;'>{it.note}</td></tr>"
            for it in items[:25]
        )
        table_html = (
            "<table border='1' cellspacing='0' cellpadding='4' "
            "style='border-collapse:collapse;width:100%;margin-top:8px;'>"
            "<tr style='background:#f1f5f9;'><th>Codice</th>"
            "<th>Descrizione</th><th>Misura (UM)</th><th>P. Unit.</th>"
            "<th>Totale (€)</th><th>Regola di calcolo e note</th></tr>"
            f"{rows_html}</table>"
        )

        if warnings:
            warnings_html = "<br/>".join(
                f"- {warning}" for warning in warnings[:30]
            )
            self.measurement_feedback.setHtml(
                f"<h3>Computo salvato: Run #{run_id} ({len(items)} "
                f"elementi)</h3><p style='color:#b91c1c;'><b>Avvisi / "
                f"Anomalie:</b><br/>{warnings_html}</p>{table_html}"
            )
        else:
            self.measurement_feedback.setHtml(
                f"<h3>Computo salvato: Run #{run_id} ({len(items)} "
                f"elementi)</h3><p style='color:#15803d;'><b>✓ Computo "
                f"calcolato correttamente senza anomalie.</b></p>{table_html}"
            )

        self.tabs.setCurrentWidget(self.report_tab)
        self.log(
            f"Creato run di computo #{run_id} dal layer '{layer.name()}' con "
    ...
    def refresh_accounting_tab(self):
        ...
        if not run_id:
            ...
            self.accounting_certificate_browser.setHtml(
                "<p>Nessun run disponibile per la contabilità.</p>"
            )
            return
        ...
        for row_index, row in enumerate(journal_rows):
            for column_index, key in enumerate(
                ("work_date", "title", "weather", "workers", "description")
            ):
                item = QTableWidgetItem(str(row.get(key) or ""))
                if column_index == 0:
                    item.setData(QT_USER_ROLE, row["id"])
                self.accounting_journal_table.setItem(
                    row_index, column_index, item
                )
        self.accounting_journal_table.resizeColumnsToContents()
        self.accounting_journal_table.horizontalHeader().setSectionResizeMode(
            4, header_stretch_mode()
        )
        ...
        for row_index, row in enumerate(sal_rows):
            ...
            for column_index, value in enumerate(values):
                ...
                if column_index == 0:
                    item.setData(QT_USER_ROLE, row["id"])
                self.accounting_sal_table.setItem(
                    row_index, column_index, item
                )
        self.accounting_sal_table.resizeColumnsToContents()

        self.accounting_certificate_browser.setHtml(
            self._build_accounting_certificate_preview(summary_rows)
        )

    def _build_accounting_certificate_preview(
        ...
        retention_line = (
            f"<p><b>Ritenuta:</b> {sal_data['retention_percent']:.2f}% = "
            f"€ {sal_data['retention_amount']:.2f}</p>"
        )
        vat_line = (
            f"<p><b>IVA:</b> {sal_data['vat_percent']:.2f}% = "
            f"€ {sal_data['vat_due']:.2f}</p>"
        )
        total_due_line = (
            "<p><b>Totale certificato:</b> "
            f"<span style=\"font-size:15px\"><b>"
            f"€ {sal_data['total_due']:.2f}</b></span></p>"
        )
        return f"""
            ...
            {retention_line}
            <p><b>Certificato a tutto il SAL:</b>
            € {sal_data['certified_to_date']:.2f}</p>
    ...
    def _sal_record_with_totals(
        self, sal_record: dict, summary_rows: list[dict]
    ) -> dict:
        ...
        totals = compute_sal_totals(
            ...
            retention_percent=float(
                sal_record.get("retention_percent") or 0.0
            ),
    ...
    def current_sal_form_record(self) -> dict:
        return {
            ...
            "security_costs": self._safe_float(
                self.sal_security_costs_edit.text(), 0.0
            ),
            "retention_percent": self._safe_float(
                self.sal_retention_percent_edit.text(), 0.0
            ),
            "vat_percent": self._safe_float(
                self.sal_vat_percent_edit.text(), 22.0
            ),
            "previous_paid": self._safe_float(
                self.sal_previous_paid_edit.text(), 0.0
            ),
    ...
    def generate_sal_document(self):
        ...
        if not run_id:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Nessun run disponibile per generare il SAL.",
            )
            return
        sal_record = self.current_sal_form_record()
        ...
        QMessageBox.information(
            self,
            PLUGIN_NAME,
            f"SAL registrato correttamente. ID documento: {sal_id}",
        )
    ...
    def load_last_sal_document(self):
        ...
        if not run_id:
            QMessageBox.information(
                self, PLUGIN_NAME, "Nessun run disponibile."
            )
            return
        ...
        if not rows:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Non esistono SAL registrati per questo run.",
            )
            return
    ...
    def _load_sal_form(self, row: dict):
        ...
        self.sal_security_costs_edit.setText(
            str(row.get("security_costs") or 0)
        )
        self.sal_retention_percent_edit.setText(
            str(row.get("retention_percent") or 0)
        )
        ...
        if run_id:
            self.accounting_certificate_browser.setHtml(
                self._build_accounting_certificate_preview(
                    self.db.get_run_summary(run_id)
                )
            )
    ...
    def add_journal_entry(self):
        ...
        if not title:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Inserisci almeno il titolo della voce di giornale.",
            )
            return
    ...
    def delete_journal_entry(self):
        ...
        if not entry_id:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Seleziona prima una voce del giornale lavori.",
            )
            return
    ...
    def export_accounting_pdf(self):
        ...
        if not run_id:
            QMessageBox.information(
                self,
                PLUGIN_NAME,
                "Nessun run disponibile per l'export contabile.",
            )
            return
        self.save_report_profile(show_message=False)
        suggested = os.path.join(
            self.export_dir, f"contabilita_run_{run_id}.pdf"
        )
        path, _ = QFileDialog.getSaveFileName(
            self, "Esporta PDF contabilità", suggested, "PDF (*.pdf)"
        )
        if not path:
            return
        ...
        map_path = (
            self.capture_report_map(run_id)
            if self.report_include_map_checkbox.isChecked()
            else ""
        )
        ...
        QMessageBox.information(
            self, PLUGIN_NAME, f"PDF contabilità generato:\n{path}"
        )
    ...
    def export_accounting_workbook(self):
        ...
        self.save_report_profile(show_message=False)
        suggested = os.path.join(
            self.export_dir, f"contabilita_run_{run_id}.xlsx"
        )
        path, _ = QFileDialog.getSaveFileName(
            self, "Esporta XLSX contabilità", suggested, "Excel (*.xlsx)"
        )
        if not path:
            return
        metadata = self.db.get_run_metadata(run_id)
        ...
        QMessageBox.information(
            self, PLUGIN_NAME, f"File XLSX contabilità generato:\n{path}"
        )
    ...
    def export_current_pdf(self):
        ...
        if not run_id:
            QMessageBox.information(
                self, PLUGIN_NAME, "Nessun run disponibile da esportare."
            )
            return
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Esporta PDF", suggested, "PDF (*.pdf)"
        )
        if not path:
            return
    ...
    def export_current_xlsx(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Esporta XLSX", suggested, "Excel (*.xlsx)"
        )
        if not path:
            return
        ...
        details = self.db.get_run_details(run_id)
        export_xlsx(
            path, metadata, summary, details, profile=self.report_profile
        )
        self.log(f"Report XLSX esportato: {path}")
        QMessageBox.information(
            self, PLUGIN_NAME, f"File XLSX generato:\n{path}"
        )
    ...
    def load_report_profile(self):
        ...
        self.report_include_map_checkbox.setChecked(
            self.report_profile.include_map
        )
        ...
        for reference in self.report_profile.references:
            row = self.report_refs_table.rowCount()
            self.report_refs_table.insertRow(row)
            self.report_refs_table.setItem(
                row, 0, QTableWidgetItem(reference.label)
            )
            self.report_refs_table.setItem(
                row, 1, QTableWidgetItem(reference.value)
            )
    ...
    def save_report_profile(self, show_message: bool = True):
        ...
        self.report_profile = ReportProfile(
            title=self.report_title_edit.text().strip()
            ...
            map_title=self.report_map_title_edit.text().strip()
        ...
        if show_message:
            QMessageBox.information(
                self, PLUGIN_NAME, "Profilo report salvato."
            )
    ...
    def _run_layer_extent(self, run_id: int) -> QgsRectangle | None:
        ...
        layer_names = [
            name.strip()
            for name in metadata.layer_name.split(",")
            if name.strip()
        ]
        extent = None
    ...
    def capture_report_map(self, run_id: int) -> str:
        ...
        if (
            target_extent is None
            or target_extent.isNull()
            or target_extent.isEmpty()
        ):
            canvas_factory = getattr(self.iface, "mapCanvas", None)
            canvas = canvas_factory() if callable(canvas_factory) else None
            if (
                canvas is not None
                and not canvas.extent().isNull()
                and not canvas.extent().isEmpty()
            ):
        ...
        if (
            ...
            self.log(
                "Esportazione mappa non riuscita: nessuna estensione "
                "disponibile per il run."
            )
            return ""
        ...
        aspect = (
            buffered.width() / buffered.height() if buffered.height() else 1.0
        )
        ...
        try:
            ...
            map_item.attemptSetSceneRect(
                QRectF(side_margin, top_margin, map_w, map_h)
            )
            ...
            scale_bar.attemptMove(
                QgsLayoutPoint(
                    side_margin, top_margin + map_h + scale_gap, LAYOUT_UNIT_MM
                )
            )
    ...
    def create_demo_project(self):
        ...
        try:
            ...
            demo_price_list = load_price_list(
                generated["demo_price_list_path"]
            )
            imported_id = self.db.import_price_list(demo_price_list)
            self.refresh_price_lists(selected_id=imported_id)
            self.log(
                f"Prezziario demo importato e attivato: {demo_price_list.name}"
            )
        except Exception as exc:
            QMessageBox.critical(
                self,
                PLUGIN_NAME,
                f"Creazione progetto demo non riuscita:\n{exc}",
            )
        ...
        QMessageBox.information(
            self,
            PLUGIN_NAME,
            "Progetto demo creato correttamente e Prezziario Demo "
            "importato.\n\n"
            f"Progetto: {generated['project_path']}\n"
            f"GeoPackage: {generated['gpkg_path']}\n"
            f"Prezziario attivo nel plugin: "
            f"{generated['demo_price_list_path']}\n"
    ...
    def _populate_table(
        self, table: QTableWidget, rows: list[dict], keys: list[str]
    ):
    ...
    def _update_download_progress(
        ...
        if total > 0:
            ...
            self.download_progress.setValue(
                max(0, min(100, int((downloaded / total) * 100)))
            )
        else:
            self.download_progress.setRange(0, 0)
    ...
    def _set_download_result(
        self, file_path: str, source_url: str, title: str, page_url: str = ""
    ):
        ...
        source_line = (
            f'<p><b>Sorgente file:</b> <a '
            f'href="{source_url}">{source_url}</a></p>'
            if source_url
            else ""
        )
        page_line = (
            f'<p><b>Pagina ufficiale:</b> <a '
            f'href="{page_url}">{page_url}</a></p>'
            if page_url
            else ""
        )
        self.download_result_browser.setHtml(f"""
            <h3>{title}</h3>
            <p><b>File locale:</b> <a href="{file_url}">{file_path}</a></p>
            <p><b>Cartella locale:</b>
            <a href="{folder_url}">{folder_path}</a></p>
            {source_line}
            {page_line}
            """)

    def _sanitize_download_name(
        self, base_name: str, suffix: str, source_label: str = ""
    ) -> str:
